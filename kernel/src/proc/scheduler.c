...
thread_t *scheduler_add_kernel_thread(void *entry)
{
    ...
    new_thread->gs_base = (uintptr_t)new_thread;
...
{
    __asm__ ("cli");
    lapic_timer_oneshot_us(INT_VEC_SCHEDULER, 2000000);
    __asm__ ("sti");
    for (;;) __asm__ ("hlt");
...
void scheduler_handler(cpu_ctx_t *regs)
{
    ...
    vmm_set_ctx((const page_map_ctx *)next_thread->owner->pmc);
