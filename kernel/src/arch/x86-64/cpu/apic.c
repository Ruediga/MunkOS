...
void init_ioapic(void)
{
    ...
    for (size_t i = 0; i < ioapics.size; i++) {
        mmu_map_single_page_4k(&kernel_pmc, ALIGN_DOWN(((uintptr_t)ioapics.data[i]->io_apic_address + hhdm->offset), PAGE_SIZE),
            ALIGN_DOWN((uintptr_t)ioapics.data[i]->io_apic_address, PAGE_SIZE), PM_COMMON_PRESENT | PM_COMMON_WRITE);
    }

    kprintf("%s ioapic routing initialized\n", ansi_okay_string);
}
